    ...
    if ((dir = opendir(inputtedPath)) != NULL) 
    {
        ...
        while ((dirEntry = readdir(dir)) != NULL) 
        {
            if ((strcmp(dirEntry->d_name, ".") != 0) && (strcmp(dirEntry->d_name, "..") != 0))
            {
                ...
                //If we are not in our current directory and we are at a directory, then recursively call on this directory to get its subdirectories and files              
                if (strcmp(inputtedPath,".") != 0 && dirEntry->d_type == DT_DIR)
                {
